...
class GCN_Classifier(nn.Module):
    def __init__(self, layers, method='eye', cat=True):
    ...
    def get_para(self):
        return sum([list(model.parameters()) for model in self.features if not isinstance(model, MPool)], [])
    ...
    def _init_feats(self):
        ...
        for layer in self.layers:
            if isinstance(layer, int) and layer > 0:
                ...
                features.append(GCN(layers=curr_feats, last_act=nn.Sigmoid(), method=self.method, cat=self.cat))
    ...
    def forward(self, inputs, with_loss=True, A=None):
